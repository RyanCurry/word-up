...
function addNewWordSubmission(word) {
    ...
    model.wordSubmissions.forEach(function(ThisWord){
...
function checkIfWordIsReal(word) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            // Otherwise, it is not.
            var theAnswer = (response.results.length>=1)//true;
            console.log("theAnswer", theAnswer, word)
            // TODO 15
            // Update the corresponding wordSubmission in the model
            model.wordSubmissions.forEach(function(ThisWord){
                if (ThisWord.word==word) {
                    ThisWord.isRealWord=theAnswer
                }
            })
            // re-render
            render();
...
function wordSubmissionChip(wordSubmission) {
    ...
    if (wordSubmission.hasOwnProperty("isRealWord")) {
        var scoreChip = $("<span></span>").text("X").addClass("tag tag-sm wordSubmission");
        // TODO 17
...
function isDisallowedLetter(letter) {
    ...
    return model.allowedLetters.indexOf(letter)==-1 //false;
}
